...
These are recorded rather than resolved. None blocks implementation of the behavior specified in
this change; each needs a human decision before the corresponding behavior can be treated as
final.
...
2. **Per-task reminder override** — `docs/product/journeys.md` describes a changeable default
   ...
   a persistent derived indicator. This change specifies only the derived indicator. Whether an
   individual task may carry its own reminder time is deferred.
...
5. **Audit History scope confirmation** — discovery contains an unreconciled contradiction between
   ...
   flags the contradiction as still requiring firm-lead confirmation.
6. **Evaluation success measures** — quantitative baselines, targets, and acceptance measures
   remain to be established [E-003, E-007, E-011].
7. **`Needs attention` composition** — this change includes only the slice-applicable entries
   (unowned Draft Referrals, active Referrals with no Open Task, and tasks flagged
   `Due on non-business day`). The remaining entries described for the RMS release depend on
...
9. **Audit History CSV download** — `docs/product/journeys.md` describes a CSV download option on
   ...
   a CSV download; whether the download belongs in this slice is deferred.
10. **Manual task creation** — the RMS-release Tasks page allows creating manual tasks from a
