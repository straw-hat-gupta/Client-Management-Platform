...
- [ ] 1.4 Add the PostgreSQL migration runner and the first migration containing the `evaluation_environment_marker` table and its row; verify empty-database initialization and idempotent re-application against a scratch database
...
- [ ] 1.6 Add the egress boundary checks asserting (a) `apps/api` declares no outbound HTTP, mail, or third-party integration dependency and (b) no source file in `apps/api`, the domain modules, `scripts/`, or `packages/contracts` imports `node:http`, `node:https`, `node:net`, `node:tls`, `node:dgram`, `node:dns`, `node:child_process`, or `node:worker_threads`, calls global `fetch`, or uses dynamic `import()` or `require()` with a computed specifier (`specs/evaluation-environment/spec.md` — No upload, import, or external integration); verify each check fails on a deliberately introduced violation of its own kind
...
- [ ] 3.1 Create the `audit_entry` table as append-only with a non-owning application role distinct from the migration and reset role and granted only `INSERT` and `SELECT`; verify attempted `UPDATE`, `DELETE`, `TRUNCATE`, `ALTER`, and `DROP` operations are refused by the database
...
- [ ] 3.5 Implement technical request and error logging carrying only record identifiers, error classes, and a correlation id (`specs/evaluation-environment/spec.md` — Technical logs carry no business content); verify with a redaction test that submits both a validation-rejected save and a duplicate save reaching the unique constraint, using a phone number, an email address, a Client Number, and free-text note and reason values, then asserts the database driver error text and every submitted or conflicting value appear nowhere in captured log output
...
- [ ] 4.1 Implement the `associate` record and the fixed `Evaluation User` development identity injected at the single API boundary point (`design.md` D8); verify with an integration test that every audited change records the development identity as actor, and with a negative test that a request field naming the acting Platform User — actor, user, or on-behalf-of identity — in its body, query string, or headers is rejected or ignored and never changes the recorded actor; verify separately that the attempt caller/performer Associate business field is validated but never consulted for the audit actor
...
- [ ] 4.3 Implement the refusal to deactivate an Associate who owns a non-closed Referral or an Open Task, and success after reassignment; verify with integration tests covering all three scenarios under "An Associate owning work cannot be deactivated" and proving the Associate linked to the fixed `Evaluation User` identity cannot be made inactive even when it owns no work
...
- [ ] 5.1 Implement Household and Household Member records, display-name suggestion and editing, the optional mailing address, and the invariant that a Household Member belongs to only one active Household, with persistence-level enforcement where appropriate; verify with tests covering "Household and Household Member identity", "Household display name", and rejection when the same member record is added to a second active Household in `specs/households/spec.md`
...
- [ ] 7.6 Implement search on the Households and Referrals pages covering display names, member names, phone numbers, email addresses, and Client Numbers with the matching rules in `specs/households/spec.md` — "Search matching rules"; verify per-page placement, partial case-insensitive name matching, formatting-insensitive phone matching, case-insensitive email matching, exact and prefix Client Number matching, and the active-only default with its opt-in for inactive and closed records; assert that Referral Source details, Referral Owners, task assignees, Referral Statuses, outreach stages, and note text do not match
- [ ] 7.7 Deliver the `New Referral` single-form page including its optional plain-text Referral context note, and the Referrals list with its per-page search, default `Draft` and `In process` filter, primary `New Referral` action, derived Referral age column, and empty state; verify by walkthrough that no persistent global navigation search exists, that an evaluator can search Households, create one inline, and save a Draft that names its missing activation information, and with unit tests for Referral age covering an active Referral, a closed Referral, `Draft` time included, and a Draft with no Referral Received Date

## 8. Activation and first follow-up task

- [ ] 8.1 Implement the `task` record with statuses `Open`, `Completed`, `Cancelled`, derived `Overdue`, and its mandatory link to a business record; verify with tests covering "Task statuses and derived Overdue" and "Every task is linked to a business record" in `specs/tasks-and-scheduling/spec.md`
...
- [ ] 8.6 Deliver Referral detail with summary, sources, outreach progress, tasks, activity history, and the `Audit history` action; verify by walkthrough that an evaluator can activate a Draft, see exactly one correctly scheduled first follow-up task, and find no permanent deletion action for a Referral
...
- [ ] 14.2 Verify by integration test that every successful state-mutating business command in the `households`, `referrals`, `referral-outreach`, `tasks`, `reference-records`, and `identity` modules produces an Audit History entry with previous value, new value, actor, timestamp, and any required reason; exclude the `audit` module's `recordChange` mechanism, and verify that read-only queries and scheduling calculations produce no entry
...
- [ ] 15.1 Author the synthetic baseline seed data to the composition required by `specs/evaluation-environment/spec.md` — "The synthetic baseline supports the whole evaluation demonstration" (including the overdue Open Task, the task completed on the reset date, both closure-day kinds, and the pre-positioned Referral states), modelled on the sanitized spreadsheets' structures without reusing any of their values and positioning date-dependent records relative to the reset date, plus the reset script that runs outside the application; verify by a post-reset assertion that every listed baseline element is present, seeded records carry no fabricated Audit History entries for their seeding, and the Tasks page shows non-empty overdue and `Completed today` sections; verify by running reset and asserting the environment matches the baseline exactly, and by two refusal tests: reset against a database with no evaluation-environment marker row is refused, and reset with evaluation-mode configuration absent is refused
- [ ] 15.2 Verify by test that reset is not reachable as a Platform User action and produces no Audit History entry
- [ ] 15.3 Implement the three-layer production guard: the deployment script refuses to run and names the evaluation-only boundary; the API refuses to start without the evaluation-mode configuration marker; the first migration may initialize a database containing no tables in the target schema and write the evaluation-environment marker row when that configuration marker is present; the migration runner refuses any database containing a table it did not create; and API startup, the migration runner, and reset all refuse an initialized database lacking the evaluation-environment marker row. Verify separately that the guarded deploy path refuses and names the evaluation-only boundary, API startup refuses without the evaluation-mode configuration marker, empty-database initialization writes the row, and an initialized unmarked database is refused. Across the named configuration inventory in D8, assert every combination leaves the deploy-path and marker-row refusals in force, and that the refusal predicates read no configuration beyond the evaluation-mode marker
- [ ] 15.4 Implement loopback-by-default binding: the API binds a loopback interface unless an explicitly named acknowledgement names the concrete private-range address, refuses a non-loopback interface without it, always refuses wildcard and globally routable addresses, and logs the bound interface at startup; verify the default bind, unacknowledged refusal, acknowledged private-range start, startup log line, and wildcard/globally-routable refusal even with acknowledgement
- [ ] 15.5 Add the automated repository provenance check: for every free-text cell in `agent_analysis_package/`, normalize its words, generate contiguous five-word shingles, and fail if any shingle appears in seed data, fixtures, tests, or application code; also fail if application or test code reads or imports a path under `agent_analysis_package/`. A reviewed false positive SHALL be added to an explicit allowlist with its rationale recorded beside the entry. Verify failure on a copied five-word fragment and a deliberate read of that directory. Logs and screenshots remain covered by the documented synthetic-data rule, not by this automated repository check
...
- [ ] 16.2 Run one end-to-end walkthrough per bullet of the "Evaluation success" list in `docs/product/journeys.md` and verify each is demonstrable using only synthetic data. An operator SHALL run the environment reset immediately before the demonstration session, not earlier: date-dependent baseline records are positioned relative to the reset date, so a session run on a later Firm Time Zone day would find the `Completed today` section empty and the overdue positioning shifted. The operator SHALL confirm reset's marker-row and evaluation-mode refusal conditions and its required out-of-band record. Two bullets are evidenced differently and the report must say so: the first-follow-up scheduling branches are evidenced by the table-driven unit tests in task 2.2 because activation uses server "now" and one session reaches only the branch its date produces; and reset is demonstrated by the restored baseline, with an operator running the operation alongside the evaluator
