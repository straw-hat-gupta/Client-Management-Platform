...
- [ ] 14.2 Verify by integration test that every state-mutating command listed in the `design.md` D1 module-interface tables produces an Audit History entry with previous value, new value, actor, timestamp, and any required reason; read-only queries and scheduling calculations produce no entry
...
- [ ] 15.3 Implement the three-layer production guard: the deployment script refuses to run and names the evaluation-only boundary; the API refuses to start without the evaluation-mode configuration marker; the first migration may initialize an empty database and write the evaluation-environment marker row when that configuration marker is present; and API startup, the migration runner, and reset refuse a database that already contains application tables but lacks the marker row. Verify separately that empty-database initialization writes the row and that an initialized unmarked database is refused. Across the named configuration inventory in D8, assert every combination leaves the deploy-path and marker-row refusals in force, and that the refusal predicates read no configuration beyond the evaluation-mode marker
