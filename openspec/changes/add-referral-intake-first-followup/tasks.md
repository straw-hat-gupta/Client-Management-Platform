...
- [ ] 15.3 Implement the three-layer production guard: the deployment script refuses to run and names the evaluation-only boundary; the API refuses to start without the evaluation-mode configuration marker; the first migration may initialize a database containing no tables in the target schema and write the evaluation-environment marker row when that configuration marker is present; and API startup, the migration runner, and reset refuse a database that contains any table the migration runner did not create or lacks the marker row. Verify separately that the guarded deploy path refuses and names the evaluation-only boundary, API startup refuses without the evaluation-mode configuration marker, empty-database initialization writes the row, and an initialized unmarked database is refused. Across the named configuration inventory in D8, assert every combination leaves the deploy-path and marker-row refusals in force, and that the refusal predicates read no configuration beyond the evaluation-mode marker
